...
namespace VRPayment\PluginCore\Webhook\Enum;

/**
 * Defines the Webhook Listeners, linking a unique ID to a technical name.
 * This acts as the single source of truth for all listener types, and it's retrieved from the vrpayment backend.
 */
enum WebhookListener: int
{
    ...
    {
        return match ($this) {
            self::DELIVERY_INDICATION => \VRPayment\PluginCore\DeliveryIndication\State::class,
            self::MANUAL_TASK => \VRPayment\PluginCore\ManualTask\State::class,
            self::REFUND => \VRPayment\PluginCore\Refund\State::class,
            self::TOKEN => \VRPayment\PluginCore\Token\State::class,
            self::TOKEN_VERSION => \VRPayment\PluginCore\Token\Version\State::class,
            self::TRANSACTION => \VRPayment\PluginCore\Transaction\State::class,
            self::TRANSACTION_COMPLETION => \VRPayment\PluginCore\Transaction\Completion\State::class,
            self::TRANSACTION_INVOICE => \VRPayment\PluginCore\Transaction\Invoice\State::class,
            self::TRANSACTION_VOID => \VRPayment\PluginCore\Transaction\Void\State::class,
            default => null,
        };
    }
}
