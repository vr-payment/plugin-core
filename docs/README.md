...
Welcome to the documentation for the VRPayment Plugin Core library. In this documentation, we'll guide you through everything you need to know for integrating PluginCore in your shop system.
...
Install the library, wire it to your VRPayment Portal credentials, and learn the two conventions the rest of the documentation assumes: how failures are reported, and how the VRPayment Portal's global lookup data works.

**You will finish with** a configured `SdkProvider` and a verified connection to the VRPayment Portal.
...
Settling that authorization — capturing or voiding the funds, refunding all or part of it, and retrieving the invoices, packing slips and credit notes the VRPayment Portal generates.
...
Everything that runs with no customer present: processing the webhooks the VRPayment Portal sends you, managing your webhook subscriptions, charging saved tokens for subscriptions, and tracking outstanding merchant tasks.
