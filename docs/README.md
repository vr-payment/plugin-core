...
This directory contains the documentation for the various modules of the VRPayment Plugin Core library.

## Running the Examples

Every runnable script under a module's `example/` directory shares the same bootstrap (`docs/examples/Common/bootstrap.php`), which requires a VRPayment Space ID, User ID, and API Secret, set as environment variables:
...
- [Payment Method](./PaymentMethod/): Retrieving and synchronizing VRPayment Portal configurations for available payment methods.
...
- [Webhook](./Webhook/): Guide for handling asynchronous VRPayment Portal events (Processing and Management).
