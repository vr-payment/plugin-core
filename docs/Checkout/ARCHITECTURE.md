...
The Checkout Engine follows a **Hexagonal (Ports & Adapters)** style architecture. The core domain logic is decoupled from the external VRPayment SDK and the host application's storage mechanism.
...
2.  **TransactionGateway (Infrastructure / ACL):**
    ...
    * **Responsibility:** Translates internal domain objects (`TransactionContext`) into SDK-specific models (`TransactionCreate`) and communicates with the VRPayment API.
...
3.  **TransactionPersistenceInterface (Port):**
    ...
    * **Responsibility:** Handles the storage of the VRPayment Transaction ID (e.g., in `$_SESSION` or a database table).
