...
```php
use VRPayment\PluginCore\LineItem\LineItemAttribute;
use VRPayment\PluginCore\LineItem\LineItemAttributeCollection;
...
class ShopPersistenceStrategy implements TransactionPersistenceInterface
{
    public function persist(int $transactionId): void
    {
        ...
        // $this->quote->setVRPaymentTransactionId($transactionId)->save();
    }
}
```
...
$context->transactionId = $_SESSION['vrpayment_transaction_id'] ?? null; // Load existing if any
