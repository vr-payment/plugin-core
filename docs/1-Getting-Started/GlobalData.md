...
The VRPayment Portal exposes a handful of lookup lists that describe *what the VRPayment Portal itself supports*, rather than anything about a specific merchant or transaction: the currencies and languages it accepts, the payment connectors it can route through, and the label descriptors and groups used to annotate charge attempts and tokens.
...
A plugin typically needs this data to build a currency or language picker, to validate a shop's configuration against what the VRPayment Portal actually accepts, or to resolve the descriptor IDs on a [ChargeAttempt](../2-Checkout-Flow/Charge.md)'s labels into human-readable names.
...
A shop typically stores a two-letter locale (`en`) while the VRPayment Portal expects a concrete IETF variant (`en-US`). `findPrimary()` resolves that: it returns the entry sharing the given `iso2Code` whose `primaryOfGroup` is `true`, or `null` if none is marked primary for that code.
