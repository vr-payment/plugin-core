...
When the VRPayment Portal needs the merchant to act before a transaction can proceed — a risk review on a renewal, for instance — that work appears as a manual task in the space. See [Manual Task](ManualTask.md) for counting what is outstanding so your plugin can prompt them.
...
sequenceDiagram
    ...
    PluginCore->>VRPaymentAPI: readTransaction(originalId, expand: [token])
    VRPaymentAPI-->>PluginCore: Original Transaction (with Token)
    
    alt No Token Found
        PluginCore-->>Scheduler: Error: tokenizationMode was not enabled at checkout
    end

    PluginCore->>VRPaymentAPI: createTransaction(context with token)
    VRPaymentAPI-->>PluginCore: New Transaction

    PluginCore->>VRPaymentAPI: processWithToken(newTransactionId)
    VRPaymentAPI-->>PluginCore: Charge (SUCCESSFUL)
